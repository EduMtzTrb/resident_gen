    ...
    floor_bbs = [aabb(b) for b in floors_in]
    ...
    def _faces_opposite_apartment_window(wall_coord, wall_axis_is_x, outward_is_negative,
                                        seg_lo, seg_hi, max_gap):
        """Return True when a facade wall directly faces another floor plate across a gap.

        Used only for inner-facing apartment windows that look toward an opposing bar.
        The current bar self-excludes: its own boundary gives a negative gap.
        Arguments mirror _faces_core_across_gap but check floor_bbs instead of core_bbs.

        wall_axis_is_x=True  → wall at constant world-X; gap measured along X; seg in Y.
        wall_axis_is_x=False → wall at constant world-Y; gap measured along Y; seg in X.
        outward_is_negative  → outward normal points toward smaller coordinate values.
        """
        for fbb in floor_bbs:
            if wall_axis_is_x:
                opp = fbb.Max.X if outward_is_negative else fbb.Min.X
                gap = (wall_coord - opp) if outward_is_negative else (opp - wall_coord)
                if gap < EPS or gap > max_gap + EPS:
                    continue
                span_ov = min(fbb.Max.Y, seg_hi) - max(fbb.Min.Y, seg_lo)
            else:
                opp = fbb.Max.Y if outward_is_negative else fbb.Min.Y
                gap = (wall_coord - opp) if outward_is_negative else (opp - wall_coord)
                if gap < EPS or gap > max_gap + EPS:
                    continue
                span_ov = min(fbb.Max.X, seg_hi) - max(fbb.Min.X, seg_lo)
            if span_ov > EPS:
                return True
        return False
    ...
    def _corridor_face_touches_core(ori, coord, span_lo, span_hi, z0, z1):
        ...
        When True, the caller must skip the exterior window (the core perimeter
        wall emitted by emit_core_walls already covers this face).
        """
        for cb in core_bbs:
    ...
    def win_geometry(lab, span, fh, z0, core_aligned=False, small_facing_window=False):
        # Returns (win_w, w_zlo, w_zhi) for an exterior window, by space type.
        # span = usable wall width; fh = floor height; z0 = floor bottom Z.
        # core_aligned=True        → Hall/Living: standard sill instead of tall centred.
        # small_facing_window=True → Hall/Living/Bath: WIN_H height, WIN_SILL sill;
        #                            only fires on inner-facing windows opposite another bar.
        # Hall:     90 % span × 70 % height centred (or WIN_H/WIN_SILL when aligned).
        # Living:   80 % span × 70 % height centred (or WIN_H/WIN_SILL when aligned).
        # Bedroom*: 60 % span, standard sill/head (WIN_SILL + WIN_H constants) — unchanged.
        # Bath:     50 % span, 35 % height, sill 55 % (or WIN_H/WIN_SILL when facing).
        # Others (Kitchen…): fixed WIN_W, standard sill/head — unchanged.
        # Corridor windows are handled by emit_corridor_ext_windows, not here.
        MIN_DIM = EPS * 10
        _use_std = core_aligned or small_facing_window
        if lab == "Hall":
            win_w = span * 0.90
            if _use_std:
                win_h = WIN_H
    ...
    UNIT_CORR_COL = sd.Color.FromArgb(91, 175, 175, 178)
    ...
    def emit_core_walls(z0, z1, is_ground=False):
        """Emit four perimeter wall panels for every core that overlaps this zone.

        is_ground=True  (piloti zone): all four faces get a centred entry door.
        is_ground=False (residential floors): door cut only on faces adjacent to a
        floor plate — i.e., corridor/core shared walls.  Faces that bound open air
        (end elevations in the inter-bar gap) stay solid.
        Appends directly to Wall_Breps / Wall_Colors / Door_Breps without carve().

        Panel layout (no corner overlaps):
        South / North: full X span of the core, Y-thickness at cy0 / cy1.
        West  / East : X-thickness at cx0 / cx1, inner Y span (cy0+t … cy1-t).
        """
        if not compute_walls or not cores_in:
            return
        ...
        def _face(panel, void, ind, cut_door):
            """Add one core wall panel, optionally cutting a centred door."""
            if panel is None:
                return
            if not cut_door or void is None:
                if panel.IsSolid:
            ...
            if res:
                for r in res:
                    if r and r.IsSolid:
                        ...
                        Wall_Colors.append(WALL_COL)
                if ind:
                    Door_Breps.append(ind)
            else:
                warn("Core perimeter door cut failed; keeping solid core wall")
                if panel.IsSolid:
                    Wall_Breps.append(panel)
                    Wall_Colors.append(WALL_COL)
        ...
        for cb in core_bbs:
            ...
            cmx = 0.5 * (cx0 + cx1)
            cmy = 0.5 * (cy0 + cy1)
            dw_ns = min(DOOR_W, (cx1 - cx0) * 0.80)
            dw_ew = min(DOOR_W, max(cy1 - cy0 - 2.0 * t, 0.0) * 0.80)

            if is_ground:
                d_s = d_n = d_w = d_e = True
            else:
                d_s = d_n = d_w = d_e = False
                for fbb in floor_bbs:
                    if fbb.Max.Z < z0 - EPS or fbb.Min.Z > z1 + EPS:
                        continue
                    # Single-bar: floor BB surrounds the core → all faces face corridor.
                    if (fbb.Min.X < cx0 - EPS and fbb.Max.X > cx1 + EPS and
                            fbb.Min.Y < cy0 - EPS and fbb.Max.Y > cy1 + EPS):
                        d_s = d_n = d_w = d_e = True
                        break
                    x_ov = min(fbb.Max.X, cx1) - max(fbb.Min.X, cx0) > EPS
                    y_ov = min(fbb.Max.Y, cy1) - max(fbb.Min.Y, cy0) > EPS
                    # Two-bar: floor edge touches a core face.
                    if x_ov:
                        if abs(fbb.Max.Y - cy0) < EPS: d_s = True
                        if abs(fbb.Min.Y - cy1) < EPS: d_n = True
                    if y_ov:
                        if abs(fbb.Max.X - cx0) < EPS: d_w = True
                        if abs(fbb.Min.X - cx1) < EPS: d_e = True

            # South wall: full X span, Y-thickness at cy0.
            _face(
                box_brep(cx0, cx1, cy0, cy0 + t, z0, z1),
                box_brep(cmx - 0.5*dw_ns, cmx + 0.5*dw_ns,
                        cy0 - EPS, cy0 + t + EPS, z0, d_zhi),
                box_brep(cmx - 0.5*dw_ns, cmx + 0.5*dw_ns,
                        cy0 + 0.25*t, cy0 + 0.75*t, z0, d_zhi),
                d_s
            )
            # North wall: full X span, Y-thickness at cy1.
            _face(
                box_brep(cx0, cx1, cy1 - t, cy1, z0, z1),
                box_brep(cmx - 0.5*dw_ns, cmx + 0.5*dw_ns,
                        cy1 - t - EPS, cy1 + EPS, z0, d_zhi),
                box_brep(cmx - 0.5*dw_ns, cmx + 0.5*dw_ns,
                        cy1 - 0.75*t, cy1 - 0.25*t, z0, d_zhi),
                d_n
            )
            # West wall: X-thickness at cx0, inner Y span.
            _face(
                box_brep(cx0, cx0 + t, cy0 + t, cy1 - t, z0, z1),
                box_brep(cx0 - EPS, cx0 + t + EPS,
                        cmy - 0.5*dw_ew, cmy + 0.5*dw_ew, z0, d_zhi),
                box_brep(cx0 + 0.25*t, cx0 + 0.75*t,
                        cmy - 0.5*dw_ew, cmy + 0.5*dw_ew, z0, d_zhi),
                d_w
            )
            # East wall: X-thickness at cx1, inner Y span.
            _face(
                box_brep(cx1 - t, cx1, cy0 + t, cy1 - t, z0, z1),
                box_brep(cx1 - t - EPS, cx1 + EPS,
                        cmy - 0.5*dw_ew, cmy + 0.5*dw_ew, z0, d_zhi),
                box_brep(cx1 - 0.75*t, cx1 - 0.25*t,
                        cmy - 0.5*dw_ew, cmy + 0.5*dw_ew, z0, d_zhi),
                d_e
            )
    ...
    # ------------------------------------------------------------
    # Core walls for the piloti zone (core bottom → first residential floor)
    # ------------------------------------------------------------

    if compute_walls and cores_in and floors_sorted:
        first_floor_z = aabb(floors_sorted[0]).Min.Z
        core_ground_z = min(aabb(c).Min.Z for c in cores_in)
        if first_floor_z > core_ground_z + EPS:
            emit_core_walls(core_ground_z, first_floor_z, is_ground=True)
        ...
        def emit_room(rec, aid, fi, end_lo, end_hi, is_hub=False, extra_voids=None):
            ...
            if is_facade:
                ...
                if free_span >= _min_span:
                    ...
                    sfw = _faces_opposite_apartment_window(
                        cR1, not long_is_x, cR1 < cR0, amin, amax, across * 3.0)
                    ww, wzlo, wzhi = win_geometry(lab, free_span, fh, z0,
                                                core_aligned=ca, small_facing_window=sfw)
            ...
            if ext_lo:
                ...
                if free_span >= _min_span:
                    ...
                    sfw = _faces_opposite_apartment_window(
                        amin, long_is_x, True, cmin, cmax, across * 3.0)
                    ww, wzlo, wzhi = win_geometry(lab, free_span, fh, z0,
                                                core_aligned=ca, small_facing_window=sfw)
            ...
            if ext_hi:
                ...
                if free_span >= _min_span:
                    ...
                    sfw = _faces_opposite_apartment_window(
                        amax, long_is_x, False, cmin, cmax, across * 3.0)
                    ww, wzlo, wzhi = win_geometry(lab, free_span, fh, z0,
                                                core_aligned=ca, small_facing_window=sfw)
